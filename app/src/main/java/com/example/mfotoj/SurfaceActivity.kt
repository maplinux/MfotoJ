...
class SurfaceActivity : AppCompatActivity(), SurfaceHolder.Callback, Camera.PictureCallback {
    ...
    private fun requestPermissions(permissions: Array<String?>) {
        ActivityCompat.requestPermissions(this@SurfaceActivity, permissions, REQUEST_CODE)
    }
            ...
            REQUEST_CODE -> {
                for (result in grantResults) {
                    if (result == PackageManager.PERMISSION_DENIED) {
                        Toast.makeText(this@SurfaceActivity, R.string.permission_warning, Toast.LENGTH_LONG).show()
    ...
    private fun saveImage(bytes: ByteArray) {
        ...
        try {
            val fileName = "MyFoto_" + System.currentTimeMillis() + ".jpg"
            val file = File(Environment.getExternalStorageDirectory(), fileName)
            outStream = FileOutputStream(file)
            outStream.write(bytes)
            outStream.close()
            Toast.makeText(this@SurfaceActivity, "Picture Saved: $fileName", Toast.LENGTH_LONG).show()
